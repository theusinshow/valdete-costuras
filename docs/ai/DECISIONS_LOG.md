...
- **Hero P1 (dead space):** developer left the call to us ("faço o que achar melhor"). Chose NOT to reuse `fachada.webp` in the hero (it already anchors Localização and crops poorly to the 4:5 portrait panel). Instead the right column's brand signature is **framed as a deliberate atelier seal** — bordered `bg-surface/30` panel (matching the `heroImage` frame) with the dotted-V monogram, the "Valdete Costuras" wordmark, a traço-costura divider, and the real slogan `site.slogan` ("Sua roupa no ponto certo.") in Cormorant italic. The right column now reads as an object, not empty space; the sew-in choreography is preserved. A real Valdete/atelier photo (`heroImage`) remains an optional future upgrade (CNT-009).
